...
class Actor(torch.nn.Module):
    def __init__(self,statesize,actionsize,orthogonal=True,max_action=2):
        ...
        self.std = torch.nn.Linear(64,actionsize)
    ...
    def forward(self, x , pred=True):
        ...
        x = self.fc6(x)
        mean = x
        std = torch.exp(torch.clamp(std,-20,2))
        ...
        logpi = dist.log_prob(a).sum(dim=1,keepdim=True)
        logpi -= (2*(torch.log(torch.tensor(2)) - a -F.softplus(-2*a))).sum(dim=1,keepdim=True)
        return self.tanh(a)*self.max_action,logpi
