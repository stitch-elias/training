            ...
            if len(memery) == 2048:
                ...
                for _ in range(10):
                    for index in BatchSampler(SubsetRandomSampler(range(2048)),64,False):
                        with torch.no_grad():
                            Q_ = critic_target(state_ns[index],actor_target(state_ns[index]))
                            target_Q = rs[index] + decay * (1 - ds[index]) * Q_
                        Q = critic(states[index],action[index])
                        c_loss = oss_fn(Q,target_Q)
                        opt_c.zero_grad()
                        c_loss.backward()
                        opt_c.step()
                        ...
                        a_loss = -critic(states[index],actor(state[index])).mean()
                        opt_a.zero_grad()
                        a_loss.backward()
                        opt_a.step()
